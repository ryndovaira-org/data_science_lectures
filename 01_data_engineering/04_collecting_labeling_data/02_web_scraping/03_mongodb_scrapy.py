...
print(
    f"db.habr_companies_collection.insert_many(many_posts):\n"
    f"\ttype = {type(insert_many_result)}\n"
    f"\tvalue = {insert_many_result}\n"
)
many_post_ids = insert_many_result.inserted_ids
print(
    f"db.habr_companies_collection.insert_many(many_posts).inserted_ids:\n"
...
print(
    f"db.habr_companies_collection.count_documents({{}}): {db.habr_companies_collection.count_documents({})}\n"
)
...
print(
    f"db.habr_companies_collection.find_one():\n"
    f"{pprint.pformat(db.habr_companies_collection.find_one())}\n"
)

print(
    f"db.habr_companies_collection.find_one({{'name': Авито}}):\n"
...
update_many_result = db.habr_companies_collection.update_many(
    {"tags": {"$all": ["Open source"]}}, {"$set": {"name": "MONGO"}}
)
print(
