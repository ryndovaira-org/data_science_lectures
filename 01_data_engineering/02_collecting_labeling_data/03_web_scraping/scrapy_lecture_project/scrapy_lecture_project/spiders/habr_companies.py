...
class HabrCompaniesSpider(scrapy.Spider):
    ...
    def parse(self, response):
        for company in response.css("div.company-info"):
            ...
            counter_subscribers = company.css("div.stats__counter_subscribers::text")[
                0
            ].root.strip()
            counter_rating = company.css("div.stats__counter_rating::text")[
                0
            ].root.strip()
            tags = []
            ...
            yield Request(
                link,
                callback=self.parse_item_companies,
                cb_kwargs=dict(
    ...
    def parse_item_companies(self, response, counter_subscribers, counter_rating, tags):
        ...
        item["link"] = link
        item["info"] = info
        item["counter_subscribers"] = float(
            counter_subscribers.replace(",", ".")
            .replace("\xa0", "")
            .replace("k", "000")
        )
        item["counter_rating"] = int(
            counter_rating.replace(",", "").replace("\xa0", "")
        )
        item["tags"] = tags
